...
var uuidv4 = window.sessionStorage.getItem('uuid');

var getUserAgent = function getUserAgent(usrAgentString) {
  var sBrowser = 'unknown';
  var sUsrAg = usrAgentString; //The order matters here, and this may report false positives for unlisted browsers.
...
var appendMetadataToEvents = function appendMetadataToEvents(events) {
  return {
    events: events,
    metadata: {
...
module.exports = {
  ...
  appendMetadataToEvents: appendMetadataToEvents
};

},{}],6:[function(require,module,exports){
"use strict";

var _require = require('./metadata'),
    getUserAgent = _require.getUserAgent,
    appendMetadataToEvents = _require.appendMetadataToEvents;
...
function createQueue(maxSize) {
  ...
  var Queue = {
    ...
    flush: function flush() {
      var data = appendMetadataToEvents(queue);
      ...
      var binary = Serializer.stringToBinary(json);
      var buffer = Serializer.stringToBuffer(binary);
      this.clear();
      sendData("".concat(API_URL, "/events"), buffer);
    },
...
},{"./metadata":5,"./sendData":7,"./serializer":8}],7:[function(require,module,exports){
"use strict";

require("whatwg-fetch");

var sendData = function sendData(url, buffer) {
  if (false) {
    fetch(url, {
      method: "POST",
      headers: {
...
},{"whatwg-fetch":4}],8:[function(require,module,exports){
(function (Buffer){
"use strict";
...
function () {
  ...
  _createClass(Serializer, null, [{
    key: "stringToBinary",
    value: function stringToBinary(string) {
      var binaryString = '';
    ...
    key: "charToBinary",
    value: function charToBinary(chr) {
      var output = '00000000';
    ...
    key: "stringToBuffer",
    value: function stringToBuffer(encoded) {
      var BITS_IN_BYTE = 8; // Plus 1 for offset marker

      var byteLength = Math.ceil(encoded.length / 8) + 1;
      var arr = new Uint8Array(byteLength);
      var offset = 8 - encoded.length % 8;
      arr[0] = offset % 8;
      ...
      while (index < byteLength) {
        var chunk = parseInt(encoded.substr((index - 1) * BITS_IN_BYTE, BITS_IN_BYTE), 2);
        arr[index] = chunk;
        index++;
      }
    ...
    value: function bufferToString(buffer) {
      ...
      arr.forEach(function (chunk) {
        var output = '00000000' + chunk.toString(2);
...
var queue = createQueue(5000);

var getEventData = function getEventData(eType, e) {
  var timestamp = Date.now();

  switch (eType) {
    case 'link_clicks':
      return [eType, e.target.firstChild.textContent, // linkText
      e.target.href, // targetURL
      timestamp];

    case 'clicks':
      return [eType, e.target.nodeName, // target_node
      e.buttons, // buttons
      e.clientX, // x
      e.clientY, // y
      timestamp];
    ...
    case 'key_presses':
      return [eType, e.key, // key
      timestamp];

    case 'form_submissions':
      var inputs = _toConsumableArray(e.target.elements).filter(function (e) {
      ...
      return [eType, data, timestamp];

    case 'pageviews':
      {
        return [eType, window.location.href, // url
...
var addToQueue = function addToQueue(eType, e) {
  var eventObj = getEventData(eType, e);
